"""Evaluator A and B for fixed cyclic orders."""
...
from ringmin.geometry import TAU, as_order, theta
...
def chain_radius(order: tuple[int | float, ...] | list[int | float]) -> float:
    ...
    lo = 1e-9
    hi = 4.0 * len(radii) * len(radii)
    target = TAU

    lo_value = _chain_sum(lo, radii)
    hi_value = _chain_sum(hi, radii)
    if not (lo_value > target > hi_value):
        raise AssertionError(
            "chain bisection bracket failed: "
            f"order={radii!r}, low_sum={lo_value!r}, high_sum={hi_value!r}"
        )
...
def binding_structure(
    ...
    """Backward-compatible essential binding structure and floating radii."""
...
def full_radius_value(
    ...
    if not is_feasible(radii, upper):
        upper = max(upper * 2.0, 1e-8)
        while not is_feasible(radii, upper):
            upper *= 2.0
            if upper > 1e12:
