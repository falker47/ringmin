"""Pattern cyclic orders used only for comparison."""
...
def best_exhaustive_gap_insertion(
    base_order: Iterable[int | float],
    excluded: Iterable[int | float],
) -> FullResult:
    """Try every excluded order and every gap placement, returning the best full solve."""
    base = tuple(base_order)
    excluded_values = tuple(excluded)
    if not excluded_values:
        return full_radius(base)

    best: FullResult | None = None
    for excluded_perm in permutations(excluded_values):
        candidates = [base]
        for value in excluded_perm:
            next_candidates: list[tuple[int | float, ...]] = []
            for candidate in candidates:
                for gap in range(len(candidate) + 1):
                    next_candidates.append(candidate[:gap] + (value,) + candidate[gap:])
        ...
        for candidate in candidates:
            ...
            if best is None or result.R_full < best.R_full:
                best = result
    if best is None:
        raise AssertionError("gap insertion produced no candidates")
    return best
