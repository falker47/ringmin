"""Bounded exact audit, run with python -S; stdout only.

Integers/Fraction only, no production, verifier or previous-checker imports.
Cover all floors allowed by the strict parameter brackets at the fixed SIZES.
Exact angle enclosures use a separate half-angle arctangent formula.
No implicit minimizer, geometric root or permutation optimum is computed.
"""
...
C0 = Q(1, 32)
SCALE = 10**24
SIZES = tuple(range(2, 17))+(32, 45, 46, 47, 91, 92, 93, 99, 100, 101,
                            199, 200, 201, 2047, 2048, 2049)
...
def construct(m, s, q, d):
    assert all(type(x) is int for x in (m, s, q, d))
    assert m >= 2 and 0 <= s < m and 0 <= d <= q
    assert q % 2 == d % 2 == 0 and s+q+d < Q(m, 2)
...
def formula(m, s, q, d, i):
    j = (q+2-i if i <= q and i % 2 == 0 else
         2*q+d+2-i if q < i <= q+d and i % 2 == 0 else i)
    return m+1+(j+s-1) % m
...
def analytic_gates():
    u0, e0 = Q(1, 3), Q(1, 100)
    assert Q(1, 2)-AH-LH-EH == Q(27541513, 10**9) > 0
    assert 0 < EL < EH < LL < LH < u0
    assert LH+EH < (1+AL)/3 < (1+AH)/3 < 1-AH
    assert 6*LH < 2 < 7*LL and 9*AH < 1 < 10*AL
    assert 45*EH < 2 < 47*EL
    assert u0+e0 < (1+AL)/3 and e0 < EL
    assert radical_sign(XL/(1+XL), XL, Q(1)) > 0
    assert 4*AH < 3
    # Independent sign-safe recheck of the boundary entry gates.
    ET = Q(87, 2000)
    for A, x, e, expected in ((1+AL, XH, EL, -1), (1+AH, XL, ET, 1)):
        w = e/A
        p, q = (x+w)/(1+x+w), (x+w)/(1+x)
        assert 1-p-q > 0 and radical_sign(p, q, Q(1)) == expected
    assert EH-ET-ET*ET/8 == Q(8431, 32000000) > 0
    # These gates use tau_b<epsilon_b analytically, not a rounded minimizer.
    gamma = e0**3*(u0-LH)/(48*(1+AH+u0+e0)**2)
    assert gamma > 0
    print('EXACT comparison gates:', u0-LH, (1+AL)/3-u0-e0,
          (1+AL)/3-LH-EH)
    print('EXACT gamma =', gamma, '; C_2-C_b > 7*gamma/88 =', 7*gamma/88)

    assert 2 < Q(3, 2)**2  # sqrt(2)<3/2: Lip(g)<4 and ||g||<3
    assert 2-1-Q(3, 8) == Q(5, 8)  # shell sign gate
    assert Q(3, 4)**2 < Q(3, 4) and Q(3, 4)*Q(7, 4) > 1
    assert 11*4+24*3 == 116 and 1024+16*116 == 2880
    assert 1024+116 == 1140
    m = 2048
    ...
    print('PASS exact domain, entry, shell, Lipschitz, max/tie, score/root and deletion gates')
...
def bounded_checks():
    ...
    for m in SIZES:
        for s, q, d in floor_cover(m):
            ...
            assert r >= e+2
            exc = seams(m, s, q, d)
            assert len(exc) == (2 if m <= 6 else 3 if m <= 9 else 5 if d else 4)
            interior = max(q-1, 0)+max(d-1, 0)
            ...
            for i, right in enumerate(P, 1):
                ...
                for R in (Q(1), Q(m*m, 8), Q(m*m, 2), Q(2*m*m)):
                    ...
                    signs_checked += 1
                probes = set(exc) | {q+2, e, q+d//2} if d else set(exc)
    ...
    print('PASS', cases, 'floor triples;', cells, 'actual cells;', signs_checked,
          'rational branch signs;', interval_cells, 'independent angle/full-max enclosures')
...
def mutation_checks():
    m, s, q, d = 100, 10, 30, 4
    P = construct(m, s, q, d)
    assert (P[q-1], P[q]) == (112, 141)
    assert (P[q-1], P[q]) != (m+s+q, m+s+q+1)
    assert P[-1] != P[0]  # cyclic predecessor mutation
    union = construct(m, s, q+d, 0)
    assert union != P
    R = Q(2*m*m)
    branches = {angular_branch(R, i, P[i-2], p) for i, p in enumerate(P, 1)}
    assert branches == {-1, 1}  # either single-branch score loses actual constraints
    rejected = 0
    for args in ((1, 0, 0, 0), (10, 0, 3, 0), (10, 0, 2, 3), (10, 0, 0, 2)):
        try:
            construct(*args)
        except AssertionError:
            rejected += 1
    assert rejected == 4
    print('PASS wrong shared predecessor, cyclic predecessor, union reflection and single-branch mutations detected; 4 invalid inputs rejected')
